...
## 本プロジェクトの目的
 ...
 - 以上の内容を踏まえた上で、ED法をスパイクタイミング動態に発展させたTemporal EDとSTDPを用い、100%スパイク処理のみで動作するLIFニューロンのネットワークで高い正答率を得る(2025年11月9日追記)
...
- **訓練データセットの扱い**: 
  - **重要**: 全エポックで同一の訓練データセットを使用（機械学習の標準仕様）
  - エポック開始前に訓練データを1回だけサンプリング
  - 各エポックでは同じデータセットを繰り返し学習（シャッフル順序は変更可能）
  - 理由: 同じデータで繰り返し学習することで重みが最適化される
  - 非推奨: エポックごとに異なるデータをランダム抽出する方式（学習が収束しない）
