# コラムED法（Columnar ED Method） - コラム構造を持つED法

[**日本語**](README.md) | English (準備中)

金子勇氏オリジナルのED法（Error Diffusion Method）に**コラム構造**を導入したコラムED法（Columnar ED Method）を用いて、ANNの学習を行わせる実装です。<br>
...
## コラムED法（Columnar ED Method）の特徴

### 🧠 コラム構造

**コラム（Column）**とは、脳の視覚野などに見られる神経細胞の機能的なまとまりです。同じ特徴に反応するニューロン群が縦方向に並び、一つの処理単位として機能します。

本実装では、この生物学的な構造をニューラルネットワークに取り入れています。
隠れ層のニューロン数が64で、出力が10クラスの場合、全64ニューロンを有効活用するため、コラムごとのニューロン数を動的に調整します：

```
出力層（10クラス）
   ↓
各クラス用のコラムに専用のニューロン群を動的に割り当て
   ↓
隠れ層（64ニューロン）
   ├─ クラス0専用コラム: 7ニューロン (ニューロン 0-6)   ← 余剰ニューロン+1
   ├─ クラス1専用コラム: 7ニューロン (ニューロン 7-13)  ← 余剰ニューロン+1
   ├─ クラス2専用コラム: 7ニューロン (ニューロン 14-20) ← 余剰ニューロン+1
   ├─ クラス3専用コラム: 7ニューロン (ニューロン 21-27) ← 余剰ニューロン+1
   ├─ クラス4専用コラム: 6ニューロン (ニューロン 28-33) ← 基本
   ├─ クラス5専用コラム: 6ニューロン (ニューロン 34-39) ← 基本
   ├─ クラス6専用コラム: 6ニューロン (ニューロン 40-45) ← 基本
   ├─ クラス7専用コラム: 6ニューロン (ニューロン 46-51) ← 基本
   ├─ クラス8専用コラム: 6ニューロン (ニューロン 52-57) ← 基本
   └─ クラス9専用コラム: 6ニューロン (ニューロン 58-63) ← 基本

📊 コラム構成: [7, 7, 7, 7, 6, 6, 6, 6, 6, 6]
✅ 全64ニューロンを使用（使用率100%）
...
**専有ニューロン（帰属度=1.0）**
- 各コラムに専用に割り当てられたニューロン
- そのクラスの学習に強く参加
- 例: コラム0はニューロン0-6の7個を専有

**重複ニューロン（帰属度=0.1）**
- 隣接するコラムのニューロンとの重複領域
- 生物学的特徴: 脳のコラムは明確な境界ではなく、緩やかに重複
- クラス間の協調学習を促進

**隣接コラムの定義（巡回構造）**
- 物理的に隣り合うコラム（クラス番号が±1のコラム）
- **円環状配置**: 両端のコラム（0と9）も隣接関係を持つ
  - コラム0: 左隣にコラム9、右隣にコラム1
  - コラム9: 左隣にコラム8、右隣にコラム0
- **生物学的妥当性**: 大脳皮質のコラムは円環状に配置されることが多く、境界領域でも連続的な処理が行われる

**具体例（64ニューロン、10クラス、動的割り当て: [7,7,7,7,6,6,6,6,6,6]、巡回構造）**

```python
# コラム0（7ニューロン: 0-6を専有）
# 帰属度0.1: ニューロン58-63（コラム9の領域から受信、巡回）
# 帰属度1.0: ニューロン0-6（専有領域）
# 帰属度0.1: ニューロン7-13（コラム1の領域から受信）
コラム0 = [重複6個: 58-63] + [専有7個: 0-6] + [重複7個: 7-13]

# コラム1（7ニューロン: 7-13を専有）
# 帰属度0.1: ニューロン0-6（コラム0の領域から受信）
# 帰属度1.0: ニューロン7-13（専有領域）
# 帰属度0.1: ニューロン14-20（コラム2の領域から受信）
コラム1 = [重複7個: 0-6] + [専有7個: 7-13] + [重複7個: 14-20]

# コラム4（6ニューロン: 28-33を専有）
# 帰属度0.1: ニューロン21-27（コラム3の領域から受信）
# 帰属度1.0: ニューロン28-33（専有領域）
# 帰属度0.1: ニューロン34-39（コラム5の領域から受信）
コラム4 = [重複7個: 21-27] + [専有6個: 28-33] + [重複6個: 34-39]

# コラム9（6ニューロン: 58-63を専有）
# 帰属度0.1: ニューロン52-57（コラム8の領域から受信）
# 帰属度1.0: ニューロン58-63（専有領域）
# 帰属度0.1: ニューロン0-6（コラム0の領域から受信、巡回）
コラム9 = [重複6個: 52-57] + [専有6個: 58-63] + [重複7個: 0-6]
```
...
- **Version 0.0.3** (2025-11-27):
  - columnar_ed_ann_v003.py作成完了
  - **層数別最適設定の自動選択システム実装**
    * 引数なし実行で5層構成（生物学的妥当性）を自動選択
    * --hidden指定時に層数に応じた最適lr/epochsを自動設定
    * 手動指定で自動設定を上書き可能
  - **実験結果に基づくデフォルト設定**
    * 1層: [256], lr=0.1, epochs=50
    * 2層: [512,256], lr=0.0005, epochs=100 (Fashion-MNIST: 78.06%)
    * 3層: [256,128,64], lr=0.1, epochs=50
    * 4層: [128,96,80,64], lr=0.01, epochs=50 (Fashion-MNIST: 84.08%, 最高精度)
    * 5層: [128,96,80,64,64], lr=0.01, epochs=50 (Fashion-MNIST: 83.57%, デフォルト)
  - **生物学的妥当性の追求**
    * 大脳皮質の5-6層構造を模倣した5層構成をデフォルトに
    * Tanh活性化関数により深層学習が可能に（Sigmoid版+8.38pt向上）

- **Version 0.0.2** (2025-11-25～26):
  - columnar_ed_ann_v002.py作成完了
  - **Tanhスケール版活性化関数の実装**
    * 勾配消失を緩和し、深層学習に対応
    * Fashion-MNIST 4層で84.08%達成（Sigmoid版75.70%から+8.38pt向上）
  - **全層貫通コラム構造の実装**
    * コラム参加率: 可変設定（デフォルト20%、全層共通）
    * 垂直貫通構造: 各クラスのコラムが全層を貫通
  - **He初期化のオプション実装**
    * --he_init_scaleパラメータ追加
    * FReLU等の活性化関数実験に対応
  - **5層構成での学習成功**
    * 生物学的妥当性を考慮した深層構造
    * Fashion-MNIST 5層で83.57%達成
