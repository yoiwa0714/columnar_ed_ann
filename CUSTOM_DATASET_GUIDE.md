...
Gabor特徴抽出（デフォルトON）は、以下の画像形式に対応しています：

| 画像形式 | 例 | Gabor対応 |
|----------|-----|----------|
| グレースケール正方形画像 | 28×28, 32×32, 64×64 | ✓ |
| RGBカラー正方形画像 | 32×32×3, 64×64×3 | ✓（自動的にグレースケール変換） |
| 非正方形画像・非画像データ | 28×32, テーブルデータ等 | ✗（自動的にGabor無効化） |

非正方形画像や非画像データの場合、Gabor特徴抽出は自動的に無効化され、生データがそのままネットワークに入力されます。`--no_gabor` オプションで明示的に無効化することもできます。
...
**自動最適化:** 100MB以上のデータセットでは自動的にメモリマップモードを使用します
```
