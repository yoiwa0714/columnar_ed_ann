...
- 金子勇氏提唱の**ED法 (Error Diffusion Learning Algorithm)**(以下、オリジナルED法)に、人間の脳の大脳皮質に見られる**コラム構造（Columnar Architecture）**(詳細は後述)を導入することによってED法の拡張を行ったものです。
...
- また、金子勇氏のオリジナルのCソースコードには出力クラス毎に重み空間を保持する実装サンプルもありました。そこで、この方式を実装してみたのですが、以下のような問題があることが判明したため、断念しました。(以下は問題の一部です。)
  ...
  - 学習効率が悪い(学習に時間がかかる)。例えば、10クラス分類で10,000データを学習させた時点で、それぞれのクラスが学習できたデータ数は10,000/10 = 1000データにしかならない。分類するクラス数が増えるとこの傾向はさらに顕著になる。
...
- ガウス型減衰により、コラム中心から距離に応じて影響力が滑らかに減少
...
2. **participation_rate対応**: 部分参加による学習促進（v026分析: ~0.71が有効）
3. **検証ロジック**: 0.0と1.0の禁止（0.0=コラム無意味、1.0=学習不安定）
4. **3モード対応**: participation_rate（最優先）→ column_neurons（中優先）→ overlap（最低優先）

**使用方法**:
```bash
python columnar_ed_ann.py --use_circular --participation_rate 0.71 --train 3000 --test 3000 --epochs 30
```

**技術的課題**:
- **1次元円環距離の制約**: 現在の円環構造は1次元円環トポロジー（トーラス）を使用
- **ニューロン配置の限界**: 1次元配置では空間的な近接性の表現力が2次元六角格子に劣る
- **実測値**: participation_rate=0.71指定時、各クラス約29個のニューロン（期待値36個より少ない）
...
**検証状況**: 実装完了、基本動作確認済み。精度検証は今後実施予定。
...
| パラメータ | 値 | 備考 |
|-----------|-----|------|
| `--base_column_radius` | 1.0 | 基準コラム半径（パラメータ探索結果、256ニューロン層での値） |
| `--column_radius` | None | コラム影響半径（Noneなら層ごとに自動計算） |
| `--participation_rate` | 1.0 | コラム参加率（1.0=全参加・重複なし、優先度：最高） |
