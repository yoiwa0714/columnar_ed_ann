...
- 金子勇氏提唱の**ED法 (Error Diffusion Learning Algorithm)**(以下、オリジナルED法)に、人間の脳の大脳皮質に見られる**コラム構造（Columnar Architecture）**(詳細は後述)を導入することによってオリジナルED法の拡張を行ったものです。
...
| 最適pr | **0.1 (10%)** | 0.2 (20%) |
| 最適bcr | **0.4** | 1.0 |
| ニューロン/クラス | **5-6個** | 10-11個 |
| Test精度 | **84.50%** | 78.10% |
| 空間分離性 | 高（2D） | 中（1D） |
| 計算コスト | 低（スパース） | 低 |

**最新の検証結果（2025-12-14）:**
- ハニカム構造において、**base_column_radius=0.4** と **participation_rate=0.1** の組み合わせで **Test精度84.50%** を達成
- スパース表現（各クラス約5ニューロン）により、高度に選択的な特徴抽出を実現
- 従来の78.10%（pr=0.71, bcr=1.0）から+6.40ポイントの大幅改善
...
##### 隠れ層 1層 [512ニューロン] - v027（最適パラメータ確定版）:
  ...
  - コマンド
```python
python columnar_ed_ann_v027.py --train 3000 --test 1000 --epochs 30 --hidden 512 --lr 0.20 --u1 0.5 --lateral_lr 0.08 --base_column_radius 0.4 --participation_rate 0.1 --seed 42
```
...
##### 隠れ層 1層 [512ニューロン] - v027（円環構造 + 推奨設定）:
  - Test精度: **78.10%**
  - Train精度: **80.73%**
  - 検証日: 2025-12-13
  - パラメータ: pr=0.2, lr=0.2, radius=1.0, lateral_lr=0.03, u1=0.5, u2=0.3
  - コマンド
```python
python columnar_ed_ann_v027.py --use_circular --participation_rate 0.2 --lr 0.2 --column_radius 1.0 --lateral_lr 0.03 --u1 0.5 --u2 0.3 --train 3000 --test 3000 --epochs 30 --seed 42
```

**注記**: 円環構造では**participation_rate=0.2が最適**（ハニカム構造のpr=0.71とは異なる）。各クラス約10個のニューロン（合計102個、20%）が学習に参加。学習率はデータサイズに応じた調整が必須：
- **3000サンプル**: lr=0.2で78.10%達成（lr=0.7では20.33%に崩壊）
- **1000サンプル**: lr=0.7で67.20%達成（グリッドサーチ結果）

1D円環トポロジーでは高participation_rateが不安定化を引き起こすため、低めの設定が有効。ハニカム構造（2D）と同等の性能を実現。

**実験結果比較**:

| 設定 | pr | lr | データ | Test精度 | 安定性 |
|-----|----|----|--------|---------|--------|
| グリッドサーチ | 0.2 | 0.7 | 1K | 67.2% | 安定 |
| **推奨設定** | **0.2** | **0.2** | **3K** | **78.1%** | **最適** |
| 高lr設定 | 0.2 | 0.7 | 3K | 20.3% | 崩壊 |
| 高pr設定 | 0.71 | 0.2 | 3K | 48.0% | 不安定 |
| ハニカム参照 | 0.71 | 0.2 | 3K | 78.1% | 安定 |
