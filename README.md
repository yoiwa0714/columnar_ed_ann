...
- また、金子勇氏のオリジナルのCソースコードには出力クラス毎に重み空間を保持する実装サンプルもありました。そこで、この方式を実装してみたのですが、以下のような問題があることが判明したため、断念しました。(以下は問題の一部です。)
...
- そこで、出力クラスと個々のニューロンを紐付ける方法を色々と模索している中で、人間の脳の大脳皮質の視覚野などには前項「コラム構造」に記載したような特徴のあるコラム構造があることを知り、これをオリジナルED法を多クラス分類に拡張するためにうまく取り込めないかと思い試行錯誤してみた、というのが背景になります。

### 現在実装されているパラメータに関する補足

- コラムED法の実装はこれが初である(と思われる)ため、特にコラム構造に関連するパラメータに関しては知見が全くない状況です。
- そのため、パラメータの有効/無効性を判断するためにも現時点ではできるだけ多くのパラメータを実装しています。
- よって、現在実装しているパラメータであっても先々必要が無いと判断されて削除されたり、逆に新たな機能が必要となって新たなパラメータが実装されるという可能性もあります。
...
![ハニカム構造の空間配置](important_images/hexagonal_column_structure_no_origin.png)

**特徴:**
- 23×23の六角格子上に512ニューロンを配置
- 10クラスを2-3-3-2パターンでグリッド中心に配置
- 各クラスが36-37個のニューロンに均等アクセス（pr=0.71）
- 2次元空間により、クラス間の空間的分離が明確
- ガウス型減衰により、コラム中心から距離に応じて影響力が滑らかに減少 (詳細は後述)

#### 円環構造（1次元円形配置）

![円環構造の空間配置](important_images/circular_column_structure_no_origin.png)

**特徴:**
- 512ニューロンを円周上に配置（1次元トーラストポロジー）
- 10クラスを等間隔＋中心化オフセットで配置
- 各クラスが約10個のニューロンにアクセス（pr=0.2が最適）
- 1次元配置のため、低participation_rateが適している
...
![ハニカム構造の中心化配置](important_images/hexagonal_column_structure.png)

![円環構造の中心化配置](important_images/circular_column_structure.png)
...
**パラメータ探索の結果:**
```
participation_rate と Test精度の関係（base_column_radius=0.4固定）:
...
![ハニカム構造のアフィニティ分布](important_images/hexagonal_column_structure.png)

![円環構造のアフィニティ分布](important_images/circular_column_structure.png)
...
##### 隠れ層 1層 [512ニューロン] - v027（最適パラメータ確定版）:
  ...
  - Train精度: **87.60%**
  - 検証日: 2025-12-14
  - 最適パラメータ: **base_column_radius=0.4**, **participation_rate=0.1**
  - コマンド
```python
python columnar_ed_ann.py --train 3000 --test 1000 --epochs 30 --hidden 512 --lr 0.20 --u1 0.5 --lateral_lr 0.08 --base_column_radius 0.4 --participation_rate 0.1 --seed 42
```

**注記**: 上記の結果は、2025-12-14に実施したパラメータ探索により確定した最適値での検証結果です。
- **base_column_radius=0.4**: コラム半径探索（0.4-1.4）の結果、0.4が最適（Test精度: 0.4=83.0%, 0.6=82.0%, 1.0=79.4%）
...
##### 隠れ層 1層 [512ニューロン] - v027（円環構造 + 最適パラメータ確定版）:
  ...
  - Train-Test gap: **2.43%**
  - 検証日: 2025-12-14
  - 最適パラメータ: **base_column_radius=0.4**, **participation_rate=0.1** (ハニカム構造と同一)
  - その他パラメータ: lr=0.2, lateral_lr=0.08, u1=0.5
  - コマンド
```python
python columnar_ed_ann.py --use_circular --train 3000 --test 1000 --epochs 30 --hidden 512 --lr 0.20 --u1 0.5 --lateral_lr 0.08 --base_column_radius 0.4 --participation_rate 0.1 --seed 42
```

**注記**: 円環構造においても**participation_rate=0.1**と**base_column_radius=0.4**が最適であることが確認されました。これにより、スパース表現の原理が**コラム構造の種類（ハニカム/円環）に依存しない普遍的な最適値**であることが実証されました。
...
2. **participation_rate対応**: 部分参加による学習促進（最適値はpr=0.2と判明）
...
- **最適participation_rate**: 1D構造ではpr=0.2が最適（2D構造のpr=0.71とは対照的）
- **学習率の重要性**: データサイズに応じた調整が必須（1Kサンプル: lr=0.7、3Kサンプル: lr=0.2）

**今後の方向性（2次元拡張の可能性）**:
- 円環を2次元トーラス（ドーナツ型）に拡張し、ハニカム構造と同様の空間配置を実現
- 1次元円環を2次元平面に螺旋配置し、近接クラスが空間的に近くなるように調整
- 極座標系（r, θ）での配置により、中心からの距離と角度で2次元ガウス分布を適用

**検証状況**: 実装完了、精度検証完了（78.10%達成）。ハニカム構造と同等の実用性を確認。
...
グリッドサーチにより得られた「隠れ層数ごとのパラメータ」をデフォルト値としてコード内のテーブルで保持しています。そのため、ED法関連やコラム関連のパラメータを全く指定しなくても、ある程度高い学習精度が得られるようになっています。

1. 隠れ層が1層から5層の構成の場合のパラメータを内部テーブルで保持しています。

2. 実行時に隠れ層の層数が自動判別され、このテーブルに設定してあるパラメータが自動的に使用されます。

3. 前項のテーブルのパラメータのうち、隠れ層が1層と2層の構成についてはグリッドサーチによりある程度の最適化ができています。3層から5層の構成については暫定値が設定されており、今後のバージョンアップにより学習精度が向上する可能性があります。

4. 隠れ層6層以上の構成の場合のパラメータはこのテーブルに設定されていません。隠れ層6層以上が指定された場合には隠れ層5層のパラメータが使用されます。そのため、学習精度が低い場合には--lr等のオプション機能を使ってパラメータを個々に指定していただくことになります。

5. このテーブルに設定されているパラメータは、--list_hyperparamsオプションを指定して実行することで表示させ確認することができます。
...
| パラメータ | 値 | 備考 |
|-----------|-----|------|
| `--base_column_radius` | **0.4** | 基準コラム半径（**最新パラメータ探索結果: 2025-12-14**、256ニューロン層での値） |
