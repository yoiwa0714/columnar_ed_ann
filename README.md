...
(注) 隠れ層2層以上の場合の最適パラメータの探索を行えていないため、隠れ層2層から隠れ層5層までの構成の場合のパラメータには仮のもの設定してあります。ただし、このパラメータで実行すると学習精度は低くなる可能性が高いと思われます。なお、隠れ層2層構成から隠れ層5層構成までの構成に対して設定されているパラメータは--list_hyperparamsオプションを指定して実行することで確認することができます。
