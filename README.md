...
(参考)
実装内容と合わせて考えると、ヒートマップリアルタイム表示に表示される隠れ層の左上角辺りの赤く表示される部分がコラムの活動状況を表示していると考えています。
...
グリッドサーチにより得られた隠れ層1層の場合の最適パラメータをデフォルト値としてコード内に設定済みです。

(注1) 隠れ層2層から隠れ層5層の構成の場合のパラメータはテーブルに設定してあります。--use_hyperparamsを指定することで隠れ層の層数によってこのテーブルに設定してあるパラメータを呼び出し、使用することができます。

(注2) 前項のテーブルのパラメータは最適パラメータ探索を行えていないことから、このテーブルに設定してあるパラメータで学習を実行した場合には学習精度が低くなる可能性がありますので、ご注意ください。

(注3) 隠れ層6層以上の場合のパラメータはこのテーブルに設定していないため、--lr等のオプション機能を使ってパラメータを手動で設定していただくことになります。

(注4) このテーブルに設定されているパラメータは--list_hyperparamsオプションを指定して実行することで確認することができます。
