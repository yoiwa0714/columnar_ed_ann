...
学習のフォワード処理では、コラムED法とリザバーコンピューティング(に近い方式)とが協調動作しています。

#### リザバーコンピューティングとは

リザバーコンピューティングは、ネットワークの大部分を固定ランダム重み（リザバー）として保持し、出力層と少数のニューロンのみを学習させる方式です。

#### コラムED法におけるリザバーコンピューティング

本実装では、`column_neurons=1`の設定により、以下のようにリザバーコンピューティングが実現されています：
...
#### コラムED法とリザバーコンピューティングの役割分担

本実装では、コラムED法とリザバーコンピューティングが以下のように協調して動作します：

| 処理段階 | 役割 | 説明 |
|---------|------|------|
| **ネットワーク構造** | コラムED法（オリジナルED法） | 多層ネットワーク構造、E/Iペア、コラム構造による拡張 |
| **学習（フォワード処理）** | コラムED法 + リザバーコンピューティング | **コラムED法**: 順伝播の枠組み（活性化関数、E/Iペアなど）<br>**リザバーコンピューティング**: 固定ランダム重み（502個）による高次元特徴抽出<br>**協調動作**: リザバー（502個）が固定重みで入力を射影、コラム（10個）が学習可能な重みで調整 |
