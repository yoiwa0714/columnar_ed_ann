...
- 学習のフォワード処理部分では、コラムED法とリザーバコンピューティング(詳細は後述)とが協調動作しています。
...
学習のフォワード処理では、コラムED法とリザーバコンピューティング(に近い方式)とが協調動作しています。

#### リザーバコンピューティングとは

リザーバコンピューティングは、ネットワークの大部分を固定ランダム重み（リザーバ）として保持し、出力層と少数のニューロンのみを学習させる方式です。

#### コラムED法におけるリザーバコンピューティング

本実装では、`column_neurons=1`の設定により、以下のようにリザーバコンピューティングが実現されています：

**512ニューロン構成の例（10クラス分類）**:
- **学習ニューロン**: 10個（各クラス1個 × 10クラス）← コラムに参加
- **固定ランダム重み（リザーバ）**: 502個（512 - 10）
- **学習比率**: 約2%（10/512）

**動作原理**:
1. 入力データが512個全てのニューロンで処理される
2. 502個のリザーバニューロンは固定ランダム重みで高次元特徴を抽出
...
- リザーバの固定ランダム重みが汎化性能に貢献

#### コラムED法とリザーバコンピューティングの役割分担

本実装では、コラムED法とリザーバコンピューティングが以下のように協調して動作します：

| 処理段階 | 役割 | 説明 |
|---------|------|------|
| **ネットワーク構造** | コラムED法（オリジナルED法） | 多層ネットワーク構造、E/Iペア、コラム構造による拡張 |
| **学習（フォワード処理）** | コラムED法 + リザーバコンピューティング | **コラムED法**: 順伝播の枠組み（活性化関数、E/Iペアなど）<br>**リザーバコンピューティング**: 固定ランダム重み（502個）による高次元特徴抽出<br>**協調動作**: リザーバ（502個）が固定重みで入力を射影、コラム（10個）が学習可能な重みで調整 |
| **学習（重みの更新）** | コラムED法（オリジナルED法のアミン拡散） | コラムニューロン（10個）と出力層のみをアミン拡散で更新<br>リザーバ（502個）は更新なし |

**この役割分担により実現される利点**:
- **生物学的妥当性**: ED法のアミン拡散メカニズムを維持（誤差逆伝播を使わない）
- **計算効率**: リザーバによる学習対象の限定（98%のニューロンは固定）
...
- 512ニューロンで構成される層で、column_neurons=1 と指定した場合:
  ...
  - 残りの502個のニューロンは固定ランダム重み（リザーバ）として機能
...
| `--use_circular` | - | コラムを2次元円環配置に（デフォルトはハニカム、フラグ） |
| `--overlap` | 0.0 | コラム間の重複度（0.0-1.0、廃止予定、現在は無効） |
