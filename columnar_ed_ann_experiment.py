#!/usr/bin/env python3
"""
Columnar ED-ANN v051

columnar_ed_ann_v051.py

本ファイルは今後の開発対象である。

学習に成功した生物学的に妥当な機能のみを含む:
- コラムED法（Error Diffusion法 + コラム構造）
- Gabor特徴抽出（V1単純型細胞モデル）
- HyperParams(YAML)自動適用（1-6層最適化）
- 3系統学習率モード（output_lr / non_column_lr / column_lr）
- NC最近傍クラス帰属学習（--nc_nearest_learning, --nc_amine_strength）
- 動的シナプス刈り込み（--dynamic_pruning_fs, 開始/終了epoch指定）
- データ拡張（事前拡張/オンライン拡張）
- 可視化/ヒートマップ保存

v050/v051開発期間（2026年3月19日〜24日）で試行し断念した機能:
  ヘブ学習則、HC水平結合（加算型/乗算型）、側抑制、密コラム、
  三因子学習、最近傍コラム帰属、抑制性介在ニューロン等。
  バックアップ: code_backup/v051_cortex_mode_backup_20260324/
"""
...
# モジュールインポート
from modules_experiment.hyperparameters import HyperParams
from modules_experiment.data_loader import load_dataset, get_class_names, resolve_dataset_path, load_custom_dataset
from modules_experiment.ed_network import RefinedDistributionEDNetwork
from modules_experiment.visualization_manager import VisualizationManager
from modules_experiment.data_augmentation import augment_batch, create_augmented_dataset
...
def parse_args():
    ...
    init_group.add_argument('--init_scales', type=str, default=None,
                          help='層別初期化スケール係数（カンマ区切り、例: 0.3,0.5,1.0）\n'
                                ...
                                '  2層: [0.7, 1.8, 0.8]\n'
                                '  3層: [0.7, 1.8, 1.8, 0.8]\n'
                                '  4層: [0.7, 1.8, 1.8, 1.8, 0.8]\n'
                                '  5層: [0.7, 1.8, 1.8, 1.8, 1.8, 0.8]\n'
...
def main():
    ...
    def parse_lr_list(text, name, n_layers, allow_single_expand=True):
        ...
        if len(values) != n_layers:
            raise ValueError(
                f"--{name} には {n_layers} 個の値が必要です（指定: {len(values)}個 {values}）"
            )
        for i, v in enumerate(values):
            if v < 0:
                raise ValueError(f"--{name} の値は0以上である必要があります: Layer {i}={v}")
        return values
    ...
    # ========================================
    # 1.8.5 3系統学習率モード / 旧互換変換モード
    # ========================================
    output_lr_specified = is_arg_specified('output_lr')
    non_column_lr_specified = is_arg_specified('non_column_lr')
    column_lr_specified = is_arg_specified('column_lr')
    three_lr_mode = output_lr_specified or non_column_lr_specified or column_lr_specified
    legacy_mode = is_arg_specified(['lr', 'learning_rate']) or is_arg_specified('column_lr_factors')

    cfg_output_lr = config.get('output_lr', args.lr) if 'config' in locals() else args.lr
    cfg_non_column_lr = config.get('non_column_lr', [cfg_output_lr] * n_layers) if 'config' in locals() else [args.lr] * n_layers
    cfg_column_lr = config.get('column_lr', None) if 'config' in locals() else None
    cfg_column_lr_factors = config.get('column_lr_factors', None) if 'config' in locals() else None

    # YAML値の正規化（層数揃え）
    if isinstance(cfg_non_column_lr, (int, float)):
        cfg_non_column_lr = [float(cfg_non_column_lr)] * n_layers
    if len(cfg_non_column_lr) < n_layers:
        cfg_non_column_lr = list(cfg_non_column_lr) + [cfg_non_column_lr[-1]] * (n_layers - len(cfg_non_column_lr))
    if len(cfg_non_column_lr) > n_layers:
        cfg_non_column_lr = list(cfg_non_column_lr)[:n_layers]

    if cfg_column_lr is not None:
        if isinstance(cfg_column_lr, (int, float)):
            cfg_column_lr = [float(cfg_column_lr)] * n_layers
        if len(cfg_column_lr) < n_layers:
            cfg_column_lr = list(cfg_column_lr) + [cfg_column_lr[-1]] * (n_layers - len(cfg_column_lr))
        if len(cfg_column_lr) > n_layers:
            cfg_column_lr = list(cfg_column_lr)[:n_layers]

    if cfg_column_lr_factors is not None:
        if isinstance(cfg_column_lr_factors, (int, float)):
            cfg_column_lr_factors = [float(cfg_column_lr_factors)] * n_layers
        if len(cfg_column_lr_factors) < n_layers:
            cfg_column_lr_factors = list(cfg_column_lr_factors) + [cfg_column_lr_factors[-1]] * (n_layers - len(cfg_column_lr_factors))
        if len(cfg_column_lr_factors) > n_layers:
            cfg_column_lr_factors = list(cfg_column_lr_factors)[:n_layers]

    if cfg_column_lr is None and cfg_column_lr_factors is not None:
        cfg_column_lr = [cfg_non_column_lr[i] * cfg_column_lr_factors[i] for i in range(n_layers)]

    # 新モード（3系統）優先
    if three_lr_mode:
        try:
            effective_output_lr = args.output_lr if output_lr_specified else cfg_output_lr
            effective_non_column_lr = parse_lr_list(args.non_column_lr, 'non_column_lr', n_layers) if non_column_lr_specified else list(cfg_non_column_lr)
            if column_lr_specified:
                effective_column_lr = parse_lr_list(args.column_lr, 'column_lr', n_layers)
            elif cfg_column_lr is not None:
                effective_column_lr = list(cfg_column_lr)
            elif cfg_column_lr_factors is not None:
                effective_column_lr = [effective_non_column_lr[i] * cfg_column_lr_factors[i] for i in range(n_layers)]
            else:
                effective_column_lr = [effective_non_column_lr[i] for i in range(n_layers)]

            effective_column_lr_factors = []
            for i in range(n_layers):
                base_lr = effective_non_column_lr[i]
                effective_column_lr_factors.append(0.0 if base_lr == 0 else effective_column_lr[i] / base_lr)

            # 既存の実行経路へ反映
            args.lr = float(effective_output_lr)
            layer_lrs = list(effective_non_column_lr) + [float(effective_output_lr)]
        ...
        if layer_lrs is not None:
            effective_non_column_lr = list(layer_lrs[:-1])
            effective_output_lr = layer_lrs[-1]
        else:
            effective_non_column_lr = [args.lr] * n_layers
            effective_output_lr = args.lr

        if column_lr_factors_list is None:
            if cfg_column_lr_factors is not None:
                column_lr_factors_list = list(cfg_column_lr_factors)
            elif cfg_column_lr is not None:
                column_lr_factors_list = [0.0 if effective_non_column_lr[i] == 0 else cfg_column_lr[i] / effective_non_column_lr[i] for i in range(n_layers)]
            else:
                column_lr_factors_list = [1.0] * n_layers
    ...
    # ========================================
    if args.gabor_features:
        from modules_experiment.gabor_features import GaborFeatureExtractor
    ...
    for epoch in pbar:
        ...
        if args.batch_size is not None:
            if args.use_cupy:
                ...
                from modules_experiment.data_loader import create_tf_dataset
